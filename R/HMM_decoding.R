...
#' Furthermore this function provides estimated observation values along the most likely 
#' sequence of hidden states. See Details for more information.
#'
...
#' @param m  integer; (finite) number of states in the hidden Markov chain.
...
#' @param gamma  a matrix (\code{ncol=nrow=m}) containing values for the transition matrix 
...
#'    Possible values are \code{"global"} (for the use of the \code{Viterbi_algorithm}) 
#'    and \code{"local"} (for the use of the 
...
#' local approch.\cr
#' If \code{decoding_method="global"} is applied, the function calls 
#' \code{\link{Viterbi_algorithm}} to determine the sequence of most likely hidden states 
#' for all time points simultaneously.\cr 
#' If \code{decoding_method="local"} is applied, the function calls 
#' \code{\link{local_decoding_algorithm}} to determine the most likely hidden state for 
#' each time point seperately. 
#' 
#' \bold{Step 2:}
#' In a second step, this function links each observation to the mean of the distribution, 
#' that corresponds to the decoded state at this point in time. 
#' 
#'
#' @return
#' \code{ HMM_decoding } returns a list containing the following two components:
#' \describe{
#' \item{decoding_method}{a string object indicating the applied decoding method.}
#' \item{decoding}{a numerical vector containing the most likely sequence of hidden states 
#'       as decoded by the \code{\link{Viterbi_algorithm}} (if \code{"global"} was applied) 
#'       or by the \code{\link{local_decoding_algorithm}} (if \code{"local"} was applied).}
#' \item{decoding_distr_means}{a numerical vector of estimated oberservation values along 
#'       the most likely seuquence of hidden states (see \code{decoding} and Step 2).}
#' }
#' 
...
#' @seealso \code{\link{local_decoding_algorithm}},	\code{\link{Viterbi_algorithm}}	
#' @export
#'
#' @examples
...
HMM_decoding <- function(x, m, delta, gamma, distribution_class, distribution_theta, 
                         decoding_method = "global", discr_logL = FALSE, 
                         discr_logL_eps = 0.5)
{
 ...
 {   
 	decoding <- Viterbi_algorithm(x = x, m = m, delta = delta, gamma = gamma, 
 	                              distribution_class = distribution_class, 
 	                              distribution_theta = distribution_theta, 
 	                              discr_logL =  discr_logL, 
 	                              discr_logL_eps = discr_logL_eps)$decoding
 }
 
 if (decoding_method == "local") 
 {   
 	decoding <- local_decoding_algorithm(x = x, m = m, delta = delta, gamma = gamma, 
 	                                     distribution_class = distribution_class, 
 	                                     distribution_theta = distribution_theta, 
 	                                     discr_logL = discr_logL, 
 	                                     discr_logL_eps = discr_logL_eps)$decoding
